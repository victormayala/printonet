      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex w-72 flex-col border-r border-sidebar-border bg-toolbar-bg">
          ...
          {activeTool === "text" && !(selectedObject && selectedObject.type === "text") && (
            <div className="border-t border-sidebar-border bg-toolbar-bg">
              <button
                onClick={() => setShowCustomText(!showCustomText)}
                className="w-full flex items-center justify-between px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider hover:bg-sidebar-accent/50 transition-colors"
              >
                <span className="flex items-center gap-2"><Type className="h-3.5 w-3.5" /> Add Custom Text</span>
                <ChevronUp className={`h-4 w-4 transition-transform ${showCustomText ? "" : "rotate-180"}`} />
              </button>
              {showCustomText && (
