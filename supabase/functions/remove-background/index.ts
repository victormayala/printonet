...
serve(async (req) => {
  ...
  try {
    ...
    if (message?.images && Array.isArray(message.images)) {
      for (const img of message.images) {
        if (img.type === "image_url" && img.image_url?.url) {
          resultUrl = img.image_url.url;
          break;
        }
      }
    }

    if (!resultUrl && message?.content && Array.isArray(message.content)) {
      for (const part of message.content) {
        ...
        if (part.inline_data) {
          resultUrl = `data:${part.inline_data.mime_type};base64,${part.inline_data.data}`;
          break;
        }
      }
    }
    ...
    if (!resultUrl) {
      ...
      if (base64Match) resultUrl = base64Match[0];
    }

    if (!resultUrl) {
      return new Response(JSON.stringify({ error: "Failed to process image. Try again." }), {
        status: 422,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
