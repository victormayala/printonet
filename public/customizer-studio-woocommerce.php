...
function cs_get_stacked_preview_url_for_cart_item( $cart_item ) {
	...
	// Always pass sides so print_area data reaches the preview page
	return cs_build_stacked_preview_page_url( $thumb, $design, ! empty( $sides ) ? $sides : null );
}
...
/* =========================================================================
 * Printonet branding receiver
 *
 * Provides two REST endpoints under /wp-json/printonet/v1:
 *   - GET  /health    Public liveness check used by Printonet to confirm a
 *                     freshly cloned site is responsive before pushing branding.
 *   - POST /branding  Receives store identity + theme + asset URLs from the
 *                     Printonet dashboard. Authenticated with a shared-secret
 *                     header X-Printonet-Token that must equal the constant
 *                     PRINTONET_BRANDING_TOKEN defined in wp-config.php.
 *
 * Stored payload lives in wp_options['printonet_branding'] as a JSON-safe
 * associative array. The dashboard is the single source of truth — there is
 * no WP Admin UI for editing it.
 * ========================================================================= */
...
/**
 * Sanitize a single branding payload field. Allows null/empty so the
...
/**
 * Register Printonet REST routes.
 */
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( WP_REST_Request $request ) {
				...
				// Persist branding payload (autoload yes — we read it on every page in wp_head).
				update_option( PRINTONET_BRANDING_OPTION, $clean, true );

				// Mirror identity into core WordPress options where it makes sense.
				...
				return new WP_REST_Response(
					[
						...
						'plugin'   => 'printonet',
						'version'  => PRINTONET_PLUGIN_VERSION,
						'received' => array_keys( array_filter( $clean, fn( $v ) => $v !== '' && $v !== null ) ),
					],
					200
				);
			},
...
/**
 * Inject branding-derived CSS variables, Google Font, and favicon into <head>.
 */
add_action( 'wp_head', function () {
	...
	$primary = isset( $branding['primary_color'] ) && preg_match( '/^#[0-9a-f]{6}$/i', (string) $branding['primary_color'] )
		? $branding['primary_color']
		: '';
	$accent  = isset( $branding['accent_color'] ) && preg_match( '/^#[0-9a-f]{6}$/i', (string) $branding['accent_color'] )
		? $branding['accent_color']
		: '';
	$font    = isset( $branding['font_family'] ) && preg_match( '/^[A-Za-z0-9 \-]{1,64}$/', (string) $branding['font_family'] )
		? $branding['font_family']
		: '';
	$favicon = isset( $branding['favicon_url'] ) ? (string) $branding['favicon_url'] : '';

	if ( $font !== '' ) {
		$google_family = str_replace( ' ', '+', $font );
		echo '<link rel="preconnect" href="https://fonts.googleapis.com">' . "\n";
		echo '<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>' . "\n";
		echo '<link rel="stylesheet" href="' . esc_url( 'https://fonts.googleapis.com/css2?family=' . $google_family . ':wght@400;500;600;700&display=swap' ) . '">' . "\n";
	}
	...
	if ( $primary !== '' || $accent !== '' || $font !== '' ) {
		echo '<style id="printonet-branding-vars">:root{';
		if ( $primary !== '' ) {
			echo '--printonet-primary:' . esc_html( $primary ) . ';';
		}
		if ( $accent !== '' ) {
			echo '--printonet-accent:' . esc_html( $accent ) . ';';
		}
		if ( $font !== '' ) {
			echo "--printonet-font:'" . esc_html( $font ) . "',system-ui,-apple-system,sans-serif;";
		}
		echo '}</style>' . "\n";
	}
}, 5 );
